...
- **Корзина** (`scripts/cart.js`) — vanilla-модуль, состояние в `localStorage` (ключ `sama-lipyla-cart`, массив `{id, quantity}[]`). Экспорты: `getCart / addToCart / removeFromCart / updateQuantity / getCartCount / getCartTotal(products) / generateOrderNumber() / generateOrderMessage(cart, products, formData) / buildViberLink / buildTelegramLink`. `generateOrderNumber({ method, city })` — номер замовлення `XY-YYMMDD-HHMM-SS`, где префикс `XY` кодирует способ+місто: `S`/`D` (самовивіз/доставка) + `L`/`V` (Львів/Вінниця) → напр. `SV`, `DL`, `SL`, `DV`; хвост — дата+час формирования (без сервера/БД это простейший способ дать заказам несхожие номера — счётчика больше нет). Вызывается на submit чекаута и кладётся в `formData.orderNumber`, откуда `generateOrderMessage` печатает его строкой «Номер замовлення» под заголовком (попадает в превью и в текст Viber/Telegram). Любая мутация диспатчит `window` CustomEvent **`cart-updated`** — его слушают счётчик в Header и рендер `/cart`. Кросс-таб синк — через нативное `storage`-событие. `addToCart` вызывается делегированным листенером на `document` из `ProductsCard.astro` (по `data-product-id`), с временным фидбеком `.is-added` (1.2с).
